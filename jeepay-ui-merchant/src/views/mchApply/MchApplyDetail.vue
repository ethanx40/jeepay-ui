        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'operation'">
              <a-button
                v-if="record.channelState === 3"
                type="link"
                @click="resubmitChannel(record.channelCode)"
              >
                重新提交
              </a-button>
            </template>
...
import { getMchApplyDetail, getChannelApplyStatus, resubmitChannelApply } from '@/api/mchApply'
...
// 重新提交渠道进件
const resubmitChannel = async (channelCode: string) => {
  try {
    const response = await resubmitChannelApply(applyData.value.applyId, channelCode)
    if (response.code === 0) {
